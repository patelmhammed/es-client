package com.meesho.msearch.es.v91.repository;
...
import com.meesho.msearch.es.v91.repository.utils.EsV91WriteUtil;
...
public class EsV91Repository implements EsRepository {
    ...
    public CompletableFuture<EsWriteResponse> indexBulkDocuments(EsWriteRequest request, EsClientInfo clientInfo) {
        log.info("Write request routed to ES client version 9.1");
        EsRequestProperties requestProperties = clientInfo.getRequestProperties();
        EsV91Client client = (EsV91Client) clientInfo.getEsClient();

        var documents = request.getDocuments() == null ? Collections.<EsWriteRequest.EsIndexRecord>emptyList() : request.getDocuments();
        var bulkRequest = EsV91WriteUtil.getBulkRequestBuilder(documents, requestProperties).build();
        ...
        return EsRetryUtils.retryFuture(
                        ...
                        "EsV91Repository.indexBulkDocuments")
